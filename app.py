...
#3-camaingn simulator------> Test campaign scenarios and estimate ROI before launching
...
if st.button("Simulate"):
    ...
    st.metric("Conversions", int(conv))
    st.metric("Revenue", f"${revenue:.2f}")
    st.metric("ROI", f"{roi*100:.1f}%")
    
#4-ltv forecat-----> predict future customer lifetime value using historical behavior
st.header("LTV Forecast")

months = st.slider("Months", 1, 24, 12)
arpu = st.number_input("ARPU (Monthly)", min_value=1.0)
